...
export const Portal: React.FC<any> = ({ children, ...props }) => {
  ...
  useFrame((state) => {
    // Zoom in effect upon hovering
    content.current.scale.x = THREE.MathUtils.lerp(content.current.scale.x, 0.85 * (hovered ? 0.85 : 1), 0.1)
    content.current.scale.y = THREE.MathUtils.lerp(content.current.scale.y, 0.9 * (hovered ? 0.905 : 1), 0.1)
